    ...
    <section className="py-16">
      <div className="max-w-6xl mx-auto px-4">
        <motion.h2
          ...
          className="text-3xl md:text-4xl font-bold text-center mb-10  dark:text-white text-[#012200]"
        >
         Customer Reviews
        </motion.h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {reviews.map((r, i) => (
            <div key={i} className="bg-white dark:bg-transparent  p-6 rounded-xl shadow-lg border border-gray-200">
              ...
              <h4 className="font-semibold  text-[#012200] dark:text-[#0ec708]">— {r.name}</h4>
            </div>
          ))}
        </div>
      </div>
